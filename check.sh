...
function check_style {
	is_style_ok=1
	echo -e "\nRun cpplint\n" > result
	cpplint --filter=-legal/copyright,-readability/casting,-build/include_subdir,-runtime/threadsafe_fn,-build/header_guard,-runtime/int $( find . -name \*.h -or -name \*.c ) &>> result
	...
	echo -e "\nRun clang-tidy-12\n" >> result
	clang-tidy-12  -checks='-*,cppcoreguidelines*' $( find . -name \*.h -or -name \*.c ) -- &>> result
